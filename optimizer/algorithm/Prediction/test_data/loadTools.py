...
if __name__ == '__main__':
    ...
    start_dt = datetime(2016, 1, 16)
    end_dt = datetime(2016, 1, 17)
    energy_unit_id = '135'
    call = load_data()
    result = call.get_series_as_pd(user, pswd, server, energy_unit_id, start_dt,
                                   end_dt)
    print result

    exit()
    t = map(str, result.index)
    v = map(str, result.values)
    v = result.values
    content = zip(t, v)
    print content[-1]
    csvfile = file('/Users/Rich/Desktop/even_b.csv', 'wb')
    ...
    energy_unit_id = '143'
    sample_rate = '15s'
    start_dt = datetime(2016, 6, 11)
    end_dt = datetime(2016, 6, 11, 1)
    call = load_data()
    print call.get_series_as_pd(user, pswd, server, energy_unit_id, start_dt,
                                end_dt)

    exit()
    user = 'crowneplaza'
    pswd = user
    addr = 'app.equotaenergy.com'
    ID = '140'
    start_dt = datetime(2016, 5, 22)
    end_dt = datetime(2016, 5, 23, 12)
    call = load_data()
    print call.get_series_as_pd(user, pswd, addr, ID, start_dt, end_dt)
    exit()
